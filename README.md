...
1. Fixed Stack Dynamic
Description: Memory is allocated at compile time on the stack with a fixed size.
...
2. Stack Dynamic
Description: Memory is allocated at runtime on the stack, but the size is determined by the user during execution.
...
3. Fixed Heap Dynamic
Description: Memory is allocated on the heap for a fixed size, usually using pointers or dynamic allocation tools.
...
4. Heap Dynamic
Description: Memory is allocated on the heap, and the size can grow or shrink dynamically during runtime.
